...
const TERMS: Term[] = [
  ...
  {
    wikipedia: "Dart (programming language)",
    category: "programming-languages",
  },
  ...
  {
    wikipedia: "ACID (atomicity, consistency, isolation, durability)",
    category: "databases",
  },
...
async function fetchWikiSummary(
  ...
  try {
    const res = await fetch(url, {
      headers: {
        "User-Agent": "FutureHubDictionaryImporter/1.0 (student project)",
      },
    });
...
async function main() {
  ...
  console.log(
    `   Skipping ${done.size} already-imported slugs (from previous run)\n`,
  );
