...
const TYPE_OPTIONS: { value: ProjectType; label: string; desc: string }[] = [
  ...
  { value: "intership", label: "Internship", desc: "Internship or work placement" },
];
...
export default function CreateProjectPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (data.author) {
          setAuthorName(data.author.display_name ?? data.author.user_name ?? "");
    ...
    try {
      ...
      if (editSlug) {
        const res = await fetch(`/api/projects/${editSlug}/files`, {
          ...
          body: JSON.stringify({ file_name: file.name, file_url: r.secureUrl, file_type: file.type, file_size: file.size }),
        });
        if (res.ok) { const d = await res.json(); setFiles((p) => [...p, d.file]); }
      } else {
        setPendingImages((p) => [...p, { url: r.secureUrl, name: file.name, type: file.type, size: file.size }]);
      }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const res = editSlug
        ? await fetch(`/api/projects/${editSlug}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
          ...
          await Promise.all(
            pendingImages.map((img) =>
              fetch(`/api/projects/${data.slug}/files`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ file_name: img.name, file_url: img.url, file_type: img.type, file_size: img.size }),
              }),
            ),
          );
  ...
  }, [title, description, category, projectType, difficulty, tags, repositoryUrl, demoUrl, videoUrl, thumbnailUrl, isPublished, progress, editSlug, pendingImages, router]);

  /* ─── members ─── */
  const handleAddMember = useCallback(async () => {
    if (!editSlug) { setMemberError("Save the project first."); return; }
  ...
  const handleRemoveMember = useCallback(async (userId: string) => {
    ...
    if (!res.ok) { setMembers(snapshot); setMemberError("Failed to remove member."); }
  }, [editSlug, members]);

  const handleChangeRole = useCallback(async (userId: string, role: ProjectMemberRole) => {
    if (!editSlug) return;
  ...
  const handleAddFile = useCallback(async () => {
    if (!editSlug) { setError("Save the project first."); return; }
    if (!fileName.trim() || !fileUrl.trim()) { setError("File name and URL required."); return; }
    setSavingFile(true);
    try {
      const res = await fetch(`/api/projects/${editSlug}/files`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ file_name: fileName.trim(), file_url: fileUrl.trim(), file_type: fileType.trim() || null, file_size: fileSize ? Number(fileSize) : null }),
      });
      if (res.ok) {
        const d = await res.json();
        setFiles((p) => [d.file, ...p]);
        setFileName(""); setFileUrl(""); setFileType(""); setFileSize("");
      } else {
        const d = await res.json().catch(() => null);
  ...
  const handleDeleteFile = useCallback(async (id: number) => {
    ...
    if (!res.ok) { setFiles(snap); setError("Failed to delete file."); }
  }, [editSlug, files]);
  ...
  const nonImageFiles = files.filter((f) => !isImageUrl(f.file_url, f.file_type));

  // Media strip items (all uploadable images, not counting thumbnail which is separate)
  const galleryStripItems = [
    ...imageFiles.map((f) => ({ url: f.file_url, id: `file-${f.id}`, fileId: f.id as number | null, isPending: false })),
    ...pendingImages.map((p, i) => ({ url: p.url, id: `pending-${i}`, fileId: null as number | null, isPending: true })),
  ];

  // Current main viewer url (for gallery items)
  const activeGalleryUrl = galleryStripItems.find((g) => g.id === selectedMediaId)?.url ?? null;
  ...
  const saveLabel = saving ? "Saving…" : saveSuccess ? "Saved!" : editSlug ? "Save Changes" : "Create Project";
  ...
  return (
    <div className="relative min-h-screen pb-16 -mx-4 lg:-mx-8 bg-background text-foreground">
      ...
      <div className="relative z-10 border-b border-primary/20 bg-primary/5 px-4 py-2 text-center">
        <p className="text-xs font-medium text-primary">
          {editSlug ? "Editing — live preview. Save when ready." : "New project — fill in the details, then click Create Project."}
        </p>
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-8 grid grid-cols-1 lg:grid-cols-[84px_minmax(0,1fr)] xl:grid-cols-[84px_minmax(0,1fr)_320px] gap-6">
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-6 flex flex-col items-center gap-3">
            <Button variant="ghost" size="icon" className="rounded-full border border-border hover:bg-muted" onClick={() => router.push("/project")}>
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <Button
              variant="ghost" size="icon"
              className={`rounded-full border transition-colors ${saveSuccess ? "border-green-500/40 bg-green-500/10 text-green-500" : "border-border hover:bg-muted"}`}
              onClick={handleSave} disabled={saving}
            >
              <SaveIcon className={`h-5 w-5 ${saving ? "animate-spin" : ""}`} />
            </Button>
            {editSlug && (
              <Button variant="ghost" size="icon" className="rounded-full border border-border hover:bg-muted" onClick={() => router.push(`/project/${editSlug}`)}>
                <Eye className="h-5 w-5" />
              </Button>
            )}
            <div className="h-px w-10 bg-border my-1" />
            <div className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${isPublished ? "border-green-500/30 text-green-500 bg-green-500/10" : "border-border text-muted-foreground"}`}>
        ...
        <main className="space-y-8 min-w-0">
          ...
          <div className="flex items-center justify-between lg:hidden">
            ...
            <Button onClick={handleSave} disabled={saving} size="sm">
              <SaveIcon className={`h-4 w-4 mr-1 ${saving ? "animate-spin" : ""}`} />{saveLabel}
            </Button>
          </div>
          ...
          {error && (
            <div className="flex items-center justify-between rounded-lg border border-destructive/50 bg-destructive/10 px-3 py-2 text-sm text-destructive">
              <span>{error}</span>
              <button onClick={() => setError("")} className="ml-3"><X className="h-4 w-4" /></button>
            </div>
          )}
          ...
          <div className="space-y-0.5">
            ...
            />
            <p className="text-[10px] text-muted-foreground/40 pl-0.5">Click above to edit title</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div
              ...
              onClick={() => {
                if (selectedMediaId === "thumbnail" || (!activeGalleryUrl && selectedMediaId !== "youtube")) {
            ...
            >
              {uploadingThumbnail ? (
                ...
                <img src={activeGalleryUrl} alt="gallery" className="w-full h-full object-cover" />
              ) : thumbnailUrl ? (
                <>
                  <img src={thumbnailUrl} alt={title || "thumbnail"} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/45 transition-colors flex flex-col items-center justify-center gap-2">
                    ...
                    <span className="text-white text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity drop-shadow">Change Thumbnail</span>
                  </div>
                </>
              ) : (
              ...
              <input ref={thumbnailInputRef} type="file" accept="image/*" className="hidden" onChange={handleThumbnailUpload} disabled={uploadingThumbnail} />
            </div>

            {/* Horizontal strip */}
            <div className="flex gap-2 overflow-x-auto pb-1 scroll-smooth" style={{ scrollSnapType: "x mandatory" }}>
                ...
                >
                  ...
                  <button
                    onClick={async (e) => {
                      ...
                      if (fileId) { await handleDeleteFile(fileId); }
                      else { setPendingImages((p) => p.filter((x) => x.url !== url)); }
                    }}
                  ...
                  {isPending && (
                    <div className="absolute bottom-0 inset-x-0 bg-black/60 text-[9px] text-white/80 text-center py-0.5">unsaved</div>
                  )}
                </div>
              ))}
              ...
              >
                ...
                <input ref={galleryInputRef} type="file" accept="image/*" className="hidden" onChange={handleGalleryUpload} disabled={uploadingGallery} />
              </label>
            </div>
            ...
            <div className="flex items-center justify-between text-[10px] text-muted-foreground/50">
              {thumbnailUrl ? (
                <button onClick={() => { setThumbnailUrl(""); if (selectedMediaId === "thumbnail") setSelectedMediaId("youtube"); }} className="hover:text-destructive transition-colors">
                  Remove thumbnail
                </button>
              ) : <span />}
              {pendingImages.length > 0 && !editSlug && (
                <span className="text-amber-500/70">{pendingImages.length} photo{pendingImages.length > 1 ? "s" : ""} pending save</span>
              )}
            </div>
          </div>
          ...
          <Card className="border-border/80 bg-card/90 text-card-foreground p-5 sm:p-6 space-y-5">
            <h2 className="text-xl font-black uppercase tracking-tight">About The Project</h2>
          ...
          <Card className="border-border/80 bg-card/90 p-4 sm:p-5">
                  ...
                  <div key={m.user_id} className="flex items-center gap-3 rounded-md border border-border p-3">
                    <Avatar className="h-7 w-7 border border-border flex-shrink-0">
                      ...
                      <AvatarFallback className="text-xs">
                        {(m.profile?.display_name || m.profile?.user_name || "U").charAt(0).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium truncate">{m.profile?.display_name || m.profile?.user_name || "Unknown"}</p>
                      {m.profile?.user_name && <p className="text-xs text-muted-foreground">@{m.profile.user_name}</p>}
                    </div>
                    <Select value={m.role} onValueChange={(v) => handleChangeRole(m.user_id, v as ProjectMemberRole)}>
            ...
            <div className="flex gap-2">
              ...
              />
              <Select value={memberRole} onValueChange={(v) => setMemberRole(v as ProjectMemberRole)}>
              ...
              <Button variant="outline" size="sm" className="h-8 px-3" onClick={handleAddMember} disabled={addingMember}>
                {addingMember ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Plus className="h-3.5 w-3.5" />}
              </Button>
            </div>
            {!editSlug && (
              <p className="text-[11px] text-muted-foreground mt-2">Save the project first to add team members.</p>
            )}
          </Card>
          ...
          <Card className="border-border/80 bg-card/90 p-4 sm:p-5">
              ...
              <div className="space-y-2 mb-4">
                {nonImageFiles.map((file) => (
                  <div key={file.id} className="flex items-center gap-3 rounded-md border border-border p-3">
                    ...
                    {file.file_size && <span className="text-xs text-muted-foreground flex-shrink-0">{(file.file_size / 1024).toFixed(0)} KB</span>}
            ...
            {editSlug ? (
              <div className="rounded-md border border-dashed border-border/60 bg-muted/20 p-4 space-y-2.5">
                <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Add File</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <Input placeholder="File name" value={fileName} onChange={(e) => setFileName(e.target.value)} className="h-8 text-xs" />
                  <Input placeholder="File type (optional)" value={fileType} onChange={(e) => setFileType(e.target.value)} className="h-8 text-xs" />
                </div>
                <Input placeholder="File URL" value={fileUrl} onChange={(e) => setFileUrl(e.target.value)} className="h-8 text-xs" />
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-1.5 cursor-pointer rounded-md border border-border px-2.5 py-1.5 text-xs text-muted-foreground hover:bg-muted/50 transition-colors">
                    {uploadingAttach ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImagePlus className="h-3.5 w-3.5" />}
                    {uploadingAttach ? "Uploading…" : "Upload Image"}
                    <input ref={fileAttachInputRef} type="file" accept="image/*" className="hidden" onChange={handleAttachUpload} disabled={uploadingAttach} />
                  </label>
                  <Button variant="outline" size="sm" className="h-8" onClick={handleAddFile} disabled={savingFile || uploadingAttach}>
                    {savingFile ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : null}
                    {savingFile ? "Adding…" : "Add File"}
                  </Button>
                </div>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">Save the project first to attach files.</p>
            )}
          </Card>
        </main>
        ...
        <aside className="hidden xl:flex xl:flex-col gap-4 sticky top-[84px] self-start">
          ...
          <Card className="border-border/80 bg-card/90 p-4 text-card-foreground space-y-3">
            <p className="text-xs uppercase tracking-[0.15em] text-muted-foreground">Contributor Spotlight</p>
            <div className="flex items-start gap-3">
              <Avatar className="h-10 w-10 border border-border">
                <AvatarImage src={authorAvatar || undefined} />
                <AvatarFallback>{(authorName || user?.email?.split("@")[0] || "U").charAt(0).toUpperCase()}</AvatarFallback>
              </Avatar>
              <div className="min-w-0">
                <p className="text-sm font-semibold truncate">{authorName || user?.email?.split("@")[0] || "You"}</p>
            ...
            <div className="space-y-1">
              <p className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground"><Github className="h-3 w-3" /> Repository</p>
              <Input placeholder="https://github.com/…" value={repositoryUrl} onChange={(e) => setRepositoryUrl(e.target.value)} className="h-7 text-xs bg-muted/30 border-border/60" />
            </div>

            {/* Demo URL */}
            <div className="space-y-1">
              <p className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground"><Globe className="h-3 w-3" /> Demo URL</p>
              <Input placeholder="https://live-demo.com" value={demoUrl} onChange={(e) => setDemoUrl(e.target.value)} className="h-7 text-xs bg-muted/30 border-border/60" />
            </div>

            {/* Video URL */}
            <div className="space-y-1">
              <p className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground"><Play className="h-3 w-3" /> Video URL</p>
              <Input placeholder="YouTube link" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} className="h-7 text-xs bg-muted/30 border-border/60" />
              {youTubeId && <p className="text-[10px] text-primary/70 flex items-center gap-1"><Check className="h-2.5 w-2.5" />Embed active in preview</p>}
            </div>
            ...
            {(repositoryUrl || demoUrl) && (
              <div className="grid grid-cols-1 gap-2 pt-1">
                {repositoryUrl && (
                  <Button variant="outline" size="sm" asChild className="w-full justify-start border-border bg-muted/30 hover:bg-accent h-8">
                    <a href={repositoryUrl} target="_blank" rel="noopener noreferrer"><Github className="h-3.5 w-3.5 mr-2" />Repository</a>
                  </Button>
                )}
                {demoUrl && (
                  <Button variant="outline" size="sm" asChild className="w-full justify-start border-border bg-muted/30 hover:bg-accent h-8">
                    <a href={demoUrl} target="_blank" rel="noopener noreferrer"><Globe className="h-3.5 w-3.5 mr-2" />Live Demo</a>
                  </Button>
                )}
              </div>
            )}
          ...
          <Card className="border-border/80 bg-card/90 p-4 text-card-foreground space-y-4">
            ...
            >
              {thumbnailUrl ? (
                <img src={thumbnailUrl} alt="Thumbnail" className="w-full aspect-video object-cover transition-opacity group-hover:opacity-75" />
              ) : (
            ...
            <div className="space-y-1">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Brief</p>
              <Input
            ...
            {/* Type selector */}
            <div className="space-y-1">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Project Type</p>
                ...
                <SelectContent>
                  {TYPE_OPTIONS.map((opt) => (
                    <SelectItem key={opt.value} value={opt.value}>
                      ...
                      <span className="ml-1.5 text-muted-foreground text-[11px]">— {opt.desc}</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-1">
              ...
              <Select value={difficulty} onValueChange={(v) => setDifficulty(v as ProjectDifficulty)}>
                <SelectTrigger className={`h-7 text-xs border ${difficultyColors[difficulty]}`}><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center justify-between pt-1 border-t border-border/40">
              <div>
                <p className="text-xs font-medium">{isPublished ? "Published" : "Draft"}</p>
                <p className="text-[10px] text-muted-foreground">{isPublished ? "Visible to everyone" : "Only visible to you"}</p>
              </div>
              <button
              ...
              >
                <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${isPublished ? "translate-x-[18px]" : "translate-x-[3px]"}`} />
              </button>
            </div>
          </Card>
          ...
          {editSlug && (
            <Button variant="outline" className="w-full" onClick={() => router.push(`/project/${editSlug}`)}>
              <Eye className="h-4 w-4 mr-2" />View Project Page
            </Button>
          )}
        </aside>
      </div>
